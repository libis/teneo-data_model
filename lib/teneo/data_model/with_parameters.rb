...
module Teneo
  module DataModel
    module WithParameters
      included do
        one_to_many :parameters, class: Teneo::DataModel::Parameter,
                                 key: :host_id,
                                 primary_key: :id,
                                 conditions: { host_type: to_s },
                                 order: Sequel.asc(:name),
                                 eager: :parameter_overrides
        many_to_many :overrides, class: Teneo::DataModel::Parameter,
                                 join_table: :parameter_overrides,
                                 left_key: :from_parameter_id,
                                 right_key: :to_parameter_id,
                                 reciprocal: :overriders,
                                 conditions: { host_type: to_s },
                                 order: Sequel.asc(:name),
                                 eager: :parameter_overrides
        many_to_many :overriders, class: Teneo::DataModel::Parameter,
                                  join_table: :parameter_overrides,
                                  left_key: :to_parameter_id,
                                  right_key: :from_parameter_id,
                                  reciprocal: :overrides,
                                  conditions: { host_type: to_s },
                                  order: Sequel.asc(:name),
                                  eager: :parameter_overrides
      end
      ...
      def get_parameter(**)
        parameters_dataset.find(**).first
      end

      def add_linked_parameter(parameter, **opts)
        param = add_parameter(opts)
      ...
      def ancestor_host_paths(to_host)
        ...
        parameter_parent_hosts.map(&:ancestor_host_paths.call(to_host)).compact.reduce([]) { |r, i| r + i.unshift(self) }
      end
